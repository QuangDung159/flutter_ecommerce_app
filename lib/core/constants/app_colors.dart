// ignore_for_file: prefer_const_constructors

import 'package:flutter/material.dart';

class AppColors {
  static Color primary = Color(0xffF4BC4B);
  static Color greyScale = Color(0xff868686);
  static Color greyDark = Color(0xff2D2E32);
  static Color blackPrimary = Color(0xff333333);
  static Color yellowLight = Color(0xffFFE9BC);
  static Color orangeSecondary = Color(0xffEE7B50);
  static Color scaffoldBackgroundColor = Color(0xffF5F5F5);
  static Color border = Color(0xffDCDCDC);
  static Color greyMid = Color(0xff9C9C9C);
  static Color greyDisable = Color(0xffE9E9E9);
  static Color grey = Color(0xff828282);
}
